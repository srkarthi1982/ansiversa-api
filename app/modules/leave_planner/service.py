...
def duration(start, end, day_type):
    if day_type != "full_day": return 0.5
...
def save_entry(db,user,payload,id=None):
    ...
    if not leave_type.is_active and not id: raise HTTPException(409,"Inactive leave types cannot be used for new entries.")
    if repository.overlap(db,owner(user),payload.start_date,payload.end_date,id): raise HTTPException(409,"This leave overlaps another active leave entry.")
    item=repository.get_entry(db,owner(user),id) if id else LeaveEntry(owner_id=owner(user))
