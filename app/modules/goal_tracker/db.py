...
class GoalTrackerBase(DeclarativeBase):
    pass
...
goal_tracker_engine = create_engine(
    _build_database_url(settings.GOAL_TRACKER_DATABASE_URL),
    connect_args=_build_connect_args(settings.GOAL_TRACKER_DATABASE_URL),
    **_build_engine_kwargs(settings.GOAL_TRACKER_DATABASE_URL),
)
GoalTrackerSessionLocal = sessionmaker(
    autocommit=False,
    autoflush=False,
    bind=goal_tracker_engine,
)


def get_goal_tracker_db() -> Generator[Session, None, None]:
    db = GoalTrackerSessionLocal()
    try:
        yield db
    finally:
        db.close()
