...
class DecisionSummary(DecisionWrite):id:str;option_count:int=Field(serialization_alias="optionCount");criterion_count:int=Field(serialization_alias="criterionCount");evaluation_complete:bool=Field(serialization_alias="evaluationComplete");created_at:datetime=Field(serialization_alias="createdAt");updated_at:datetime=Field(serialization_alias="updatedAt");decided_at:datetime|None=Field(serialization_alias="decidedAt");model_config=ConfigDict(populate_by_name=True)
class DecisionDetail(DecisionSummary):options:list[OptionResponse];criteria:list[CriterionResponse];ratings:list[RatingResponse];warning:str
