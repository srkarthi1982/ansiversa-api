...
class MealPlannerBase(DeclarativeBase):
    pass
...
meal_planner_engine = create_engine(
    _build_database_url(settings.MEAL_PLANNER_DATABASE_URL),
    connect_args=_build_connect_args(settings.MEAL_PLANNER_DATABASE_URL),
    **_build_engine_kwargs(settings.MEAL_PLANNER_DATABASE_URL),
)
MealPlannerSessionLocal = sessionmaker(
    autocommit=False,
    autoflush=False,
    bind=meal_planner_engine,
)


def get_meal_planner_db() -> Generator[Session, None, None]:
    db = MealPlannerSessionLocal()
    try:
        yield db
    finally:
        db.close()
