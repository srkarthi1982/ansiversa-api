...
class RentACarBase(DeclarativeBase):
    pass
...
rent_a_car_engine = create_engine(
    _build_database_url(settings.RENT_A_CAR_DATABASE_URL),
    connect_args=_build_connect_args(settings.RENT_A_CAR_DATABASE_URL),
    **_build_engine_kwargs(settings.RENT_A_CAR_DATABASE_URL),
)
RentACarSessionLocal = sessionmaker(
    autocommit=False,
    autoflush=False,
    bind=rent_a_car_engine,
)


def get_rent_a_car_db() -> Generator[Session, None, None]:
    db = RentACarSessionLocal()
    try:
        yield db
    finally:
        db.close()
