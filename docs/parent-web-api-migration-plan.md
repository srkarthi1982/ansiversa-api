...
| Admin apps foundation | `GET/POST/PATCH/DELETE /api/v1/admin/apps...`, `GET /api/v1/admin/apps/meta` | Completed as admin apps foundation aligned to parent Astro action behavior for pagination, sorting, category metadata, key/slug duplicate blocking, URL normalization, capabilities serialization, delete behavior, and audit logging; parent web action remains unchanged |
| Admin users read foundation | `GET /api/v1/admin/users`, `GET /api/v1/admin/users/{user_id}` | Partially completed as read-only admin users foundation aligned to parent Astro action behavior for pagination, search, role/status handling, location fields, and safe response shape; admin user writes remain deferred |
...
Phase 9 correction: the previous lowercase `users`/`full_name`/`is_active` auth mismatch has been corrected in the API model. Auth now uses parent-compatible `Users` and `Roles` tables with `name`, `passwordHash`, `roleId`, and `status`. Phase 19 adds compatibility for parent web legacy `salt:hash` HMAC-SHA256 password hashes using the shared `ANSIVERSA_AUTH_SECRET`; successful legacy logins upgrade `Users.passwordHash` to Argon2. Missing/empty legacy status values are treated as active for parent compatibility, while `disabled`, `inactive`, and `suspended` users remain blocked. Parent web cookie/session parity, billing/session claims, and broader protected APIs remain deferred.
...
| Admin apps list/meta | `web/src/actions/adminApps.ts` `adminApps.list/meta` | `GET /api/v1/admin/apps`, `GET /api/v1/admin/apps/meta` | Admin | `Apps`, `Categories`, `Users` | Phase D | Completed as API foundation with `page`, `pageSize`, `q`, `categoryId`, `status`, `featuredOnly`, parent sort values, `dir`, category names, capabilities parsing, and registry normalization. Intentional API additions: search includes `description`, `sortBy`/`sortDirection` aliases are accepted, `launchStatus`/`visibility`/`pricingGate` filters are supported, and meta includes allowed values plus capability options. |
| Admin apps create/update/delete | `web/src/actions/adminApps.ts` `adminApps.create/update/delete` | `POST/PATCH/DELETE /api/v1/admin/apps...` | Admin | `Apps`, `Categories`, `AuditLogs` | Phase D | Completed as API foundation with key/slug normalization and conflict blocking, category existence validation, URL normalization, launch/visibility/pricing normalization, safe capabilities serialization, current parent delete behavior, and audit events (`admin.apps.create/update/status/featured/delete`). |
...
| Admin users list/detail | `web/src/actions/adminUsers.ts`; `web/src/pages/admin/users.astro` | `GET /api/v1/admin/users`, `GET /api/v1/admin/users/{user_id}` | Admin | `Users`, `Roles` | Phase D | Partially completed as read-only API foundation with `page`, `pageSize`, `q`, `roleId`, parent sort values, `dir`, role info, safe plan/location/avatar fields, and `404` for missing detail rows. Intentional API additions: search includes `city`, `countryCode`, and `regionCode`; filters include `status`, `plan`, `planStatus`, and `countryCode`; `sortBy`/`sortDirection` aliases are accepted. |
| Admin users create/update/delete | `web/src/actions/adminUsers.ts` `adminUsers.create/update/delete` | `POST/PATCH/DELETE /api/v1/admin/users...` | Admin | `Users`, `Roles`, `AuditLogs` | Phase D | Deferred intentionally. Must preserve email uniqueness, password hashing, status/role audit actions, and delete behavior only after write contract approval. |
...
- Admin apps foundation completed with parity to `adminApps` Astro actions where applicable. API search additionally includes `description`, meta returns form-safe allowed values, and launch/visibility/pricing filters are accepted to satisfy the Phase 17 API contract.
- Admin users read foundation partially completed with parity to `adminUsers` Astro list behavior where applicable. API search additionally includes location fields, read filters include status/plan/location values, and write/password actions remain deferred.
- Admin categories CRUD.
- Admin users write CRUD.
- Admin roles CRUD.
