...
|---|---|---|---|
| ASTRA-001 | Runtime identity and ownership | conversation metadata records the Runtime startup instance id and sessions require a runtime ownership token | `test_conversation_creation_is_deterministic_and_runtime_owned`; `test_conversation_cannot_exist_without_runtime_ownership_token` |
...
| ASTRA-003 | Bounded conversation/context | current-turn and short-context models store bounded metadata only | `test_current_turn_context_is_bounded_metadata_only`; `test_short_context_history_is_bounded_and_evicts_oldest` |
...
| ASTRA-010 | Constitutional enforcement | invalid lifecycle transitions and unsafe timestamps fail deterministically | `test_invalid_lifecycle_transition_is_rejected`; `test_context_models_reject_naive_timestamps` |
...
| ASTRA-IMP-004 | Evidence | appends evidence through `runtime.append_evidence()` | `test_evidence_is_emitted_through_runtime_core` |
