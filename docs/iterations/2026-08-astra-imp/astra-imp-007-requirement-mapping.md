...
| ASTRA-010 | Constitutional enforcement | duplicate and unknown capabilities fail deterministically; request context cannot broaden visibility; internal authority is Runtime-issued; authenticated authority cannot be self-asserted; evidence append succeeds before result release | `test_registry_rejects_duplicates_and_unknown_lookup`; `test_allowed_unknown_capability_lookup_fails_after_governed_evidence`; `test_public_and_authenticated_contexts_cannot_self_select_internal_visibility`; `test_caller_cannot_self_assert_authenticated_state`; `test_internal_visibility_requires_trusted_runtime_ownership`; `test_foreign_runtime_minted_internal_context_is_rejected`; `test_owner_issued_internal_context_remains_governance_gated`; `test_no_result_is_released_before_successful_evidence_append_and_sequence_commit` |
