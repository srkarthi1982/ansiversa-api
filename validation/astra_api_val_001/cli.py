...
from validation.astra_api_val_001.runner import SCENARIO_NAMES, run_all, run_scenario


def parser() -> argparse.ArgumentParser:
    value = argparse.ArgumentParser(
        description="ASTRA-API-VAL-001 authenticated diagnostics API validation"
    )
...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.scenario and args.scenario not in SCENARIO_NAMES:
        print(f"ASTRA-API-VAL-001 unknown scenario: {args.scenario}", file=sys.stderr)
        return 2
    try:
        results = run_all() if args.all else (run_scenario(args.scenario),)
    except Exception as exc:
        print(f"ASTRA-API-VAL-001 setup failure: {type(exc).__name__}", file=sys.stderr)
        return 3
    if args.format == "json":
