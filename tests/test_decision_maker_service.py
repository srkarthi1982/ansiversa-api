...
class DecisionMakerTests(unittest.TestCase):
 ...
 def test_list_summary_is_lightweight(self):
  found=list_decisions(self.db,self.a,page=1,page_size=10)
  row=found.items[0].model_dump(by_alias=True)
  self.assertEqual(row["title"],"Choose role")
  self.assertEqual(row["question"],"Which role should I choose?")
  self.assertNotIn("description",row)
  self.assertNotIn("outcome",row)
  self.assertNotIn("reflection",row)
  self.assertNotIn("notes",row)
  self.assertNotIn("selectedOptionId",row)
 ...
 def test_status_selected_protection_archived_and_cascade(self):
  ...
  with self.assertRaises(HTTPException):save_decision(self.db,self.a,archived.model_copy(update={"title":"Blocked edit"}),self.d.id)
  with self.assertRaises(HTTPException):delete_decision(self.db,self.a,self.d.id)
  self.d=save_decision(self.db,self.a,archived.model_copy(update={"status":"revisiting"}),self.d.id);self.assertEqual(self.d.status,"revisiting")
