...
2026-08-02 - Applied ASTRA-CHAT-001 Astra re-review corrections after PR #3 review `4838627730` at backend commit `d5c4c127c7c2fed254f7ee5463331306ca4d413b`. Removed ambient Subscription Manager private-read activation from generic Capability Discovery and Intent Resolution. Runtime-internal metadata activation is now explicit per orchestration context, with app, capability scope, and capability identity bound to the chat request instead of every Runtime-owned metadata request. Chat now binds the declared Subscription Manager capability into the declared-intent binding and `AstraIntentRequest`, requires the resolved intent to contain exactly that capability, and Read Authority Binding independently rejects mismatched, changed, or reused capability lineage. Chat response protection now uses bounded structural projection/allowlisted fields instead of keyword scanning business-data values, so valid subscription values like `1Password` and `SQL Server` can be returned while unsupported private metadata fields fail closed with a bounded chat response. Added authenticated `TestClient` coverage for the real HTTP success path through auth token, auth DB, API dependencies, non-production Runtime activation, governed read execution, seeded Subscription Manager data, foreign-user isolation, disabled activation non-success, production route absence, and unsupported capability bounds. ASTRA-CHAT-001 remains Changes Required / pending Astra re-review and is not certified. Production authorization is not approved and production remains unchanged. No frontend, provider/model/OpenAI integration, natural-language inference, RAG, embeddings, memory, adaptation, writes, generic Tool Executor, additional app adapters, schema, migration, production configuration, deployment, production authorization, merge, or certified Runtime/read-binding/read-execution weakening was introduced.

2026-08-02 - Implemented ASTRA-CHAT-001 Governed Backend Chat Orchestration after Product Owner/Astra authorization. Added a backend-only authenticated `/api/v1/astra/chat` route and deterministic declared-intent chat gateway that requires existing `AuthenticatedUserContext`, binds conversations to the exact authenticated principal, routes only certified Subscription Manager read capabilities through Conversation Context, declared Intent Resolution, metadata-only Capability Discovery, certified ASTRA-READ-AUTH-BIND-001, ASTRA-IMP-010 Read Access Authorization, ASTRA-READ-EXEC-001, and the app-owned Subscription Manager adapter, then returns bounded structured responses. Runtime-internal Capability Discovery and declared Subscription Manager `get_information` intent governance now carry the certified `subscription_manager:private_read` activation context while remaining metadata-only and non-executable. ASTRA-CHAT-001 is implemented / pending Astra review and is not certified. Production authorization is not approved and production remains unchanged. No frontend, provider/model/OpenAI integration, natural-language inference, RAG, embeddings, memory, adaptation, writes, generic Tool Executor, additional app adapters, direct SQL/database access in Astra chat, schema, migration, production configuration, deployment, production authorization, merge, or certified Runtime/read-binding/read-execution weakening was introduced.
